import React from "react";
import Link from "next/link";
import { getSuccessCaseData } from "@/lib/successCases";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { FadeIn } from "@/components/ui/fade-in";
import { DotPattern } from "@/components/ui/dot-pattern";
import { ShimmerButton } from "@/components/ui/shimmer-button";
import { cn } from "@/lib/utils";
import {
    ArrowLeft,
    Target,
    Wrench,
    BarChart3,
    MessageSquareQuote,
} from "lucide-react";

export default async function SuccessCasePage({ params }: { params: { id: string } }) {
    const { id } = params;
    const caseData = await getSuccessCaseData(id);

    if (!caseData) {
        return (
            <div className="min-h-screen bg-background flex flex-col items-center justify-center">
                <div className="text-center">
                    <h1 className="text-4xl font-bold mb-4">Case Study Not Found</h1>
                    <Link href="/case-studies">
                        <Button>Return to Case Studies</Button>
                    </Link>
                </div>
            </div>
        );
    }

    return (
        <div className="w-full min-h-screen bg-background flex flex-col">
            <main className="flex-grow pt-32 relative overflow-hidden">

                <div className="container mx-auto px-4 pb-12">
                    <div className="max-w-4xl mx-auto">
                        <FadeIn>
                            <Link
                                href="/case-studies"
                                className="inline-flex items-center text-muted-foreground hover:text-primary mb-8 group font-medium transition-colors"
                            >
                                <ArrowLeft className="w-4 h-4 mr-2 transition-transform group-hover:-translate-x-1" />
                                Back to Case Studies
                            </Link>
                        </FadeIn>

                        <FadeIn delay={0.1}>
                            <article className="bg-card/50 backdrop-blur-sm border border-border p-8 md:p-12 rounded-2xl shadow-sm">
                                <header className="mb-10 border-b border-border pb-8">
                                    <Badge className="mb-4 bg-accent/10 text-accent hover:bg-accent/20 border-accent/20">
                                        {caseData.industry}
                                    </Badge>
                                    <h1 className="text-3xl md:text-5xl font-bold text-foreground mb-4 leading-tight">
                                        {caseData.title}
                                    </h1>
                                    <p className="text-lg text-muted-foreground">
                                        Client:{" "}
                                        <span className="font-semibold text-foreground">
                                            {caseData.clientName}
                                        </span>
                                    </p>
                                </header>

                                <div className="space-y-12">
                                    <section>
                                        <h2 className="flex items-center text-xl font-bold mb-4 text-foreground">
                                            <Target className="w-6 h-6 mr-3 text-red-500" />
                                            The Challenge
                                        </h2>
                                        <p className="text-muted-foreground leading-relaxed text-lg">
                                            {caseData.challenge}
                                        </p>
                                    </section>

                                    <section>
                                        <h2 className="flex items-center text-xl font-bold mb-4 text-foreground">
                                            <Wrench className="w-6 h-6 mr-3 text-blue-500" />
                                            The Solution
                                        </h2>
                                        <p className="text-muted-foreground leading-relaxed text-lg">
                                            {caseData.solution}
                                        </p>
                                    </section>

                                    <section>
                                        <h2 className="flex items-center text-xl font-bold mb-4 text-foreground">
                                            <BarChart3 className="w-6 h-6 mr-3 text-green-500" />
                                            Key Results
                                        </h2>
                                        <ul className="space-y-3">
                                            {caseData.results.map((result, index) => (
                                                <li key={index} className="flex items-start text-muted-foreground text-lg">
                                                    <span className="font-bold mr-3 mt-1">✔️</span>
                                                    <span>{result}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </section>

                                    {caseData.testimonial && (
                                        <section className="bg-accent/5 border-l-4 border-accent p-8 rounded-r-lg my-8">
                                            <MessageSquareQuote className="w-8 h-8 text-accent/50 mb-4" />
                                            <blockquote className="text-xl italic text-foreground mb-6">
                                                "{caseData.testimonial}"
                                            </blockquote>
                                            <footer className="text-right">
                                                <p className="font-bold text-foreground">
                                                    {caseData.testimonialAuthor}
                                                </p>
                                                <p className="text-sm text-muted-foreground">
                                                    {caseData.testimonialRole}
                                                </p>
                                            </footer>
                                        </section>
                                    )}
                                </div>
                            </article>
                        </FadeIn>
                    </div>
                </div>

                {/* CTA Section - Full Width */}
                <FadeIn>
                    <section className="py-24 px-6 bg-secondary/50">
                        <div className="max-w-5xl mx-auto bg-primary rounded-2xl p-12 md:p-20 text-center relative overflow-hidden">
                            {/* DotPattern Background */}
                            <DotPattern
                                width={20}
                                height={20}
                                cx={1}
                                cy={1}
                                cr={1}
                                className={cn(
                                    "[mask-image:radial-gradient(500px_circle_at_center,white,transparent)]",
                                    "text-primary-foreground/20"
                                )}
                            />

                            {/* Decorative circles */}
                            <div className="absolute top-0 left-0 w-64 h-64 bg-accent rounded-full blur-[100px] opacity-20 -translate-x-1/2 -translate-y-1/2"></div>
                            <div className="absolute bottom-0 right-0 w-64 h-64 bg-accent/60 rounded-full blur-[100px] opacity-20 translate-x-1/2 translate-y-1/2"></div>

                            <div className="relative z-10">
                                <h2 className="text-3xl md:text-5xl font-medium text-primary-foreground tracking-tight mb-6">Want Similar Results for Your Firm?
                                </h2>
                                <p className="text-primary-foreground/70 text-lg max-w-2xl mx-auto mb-10">
                                    Schedule a discovery call to explore how we can custom-engineer an automation infrastructure for your practice.
                                </p>
                                <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                                    <Link href="/contact" className="w-full sm:w-auto">
                                        <ShimmerButton
                                            className="w-full px-8 py-3.5 rounded-2xl font-medium"
                                            background="hsl(var(--background))"
                                            shimmerColor="#0d9488"
                                            shimmerDuration="2s"
                                            shimmerSize="0.30em"
                                        >
                                            <span className="flex items-center justify-center gap-2 text-foreground">
                                                Schedule a Discovery Call
                                            </span>
                                        </ShimmerButton>
                                    </Link>
                                </div>
                                <p className="mt-6 text-xs text-primary-foreground/50">Free consultation. No commitment required</p>
                            </div>
                        </div>
                    </section>
                </FadeIn>
            </main>
        </div>
    );
}
